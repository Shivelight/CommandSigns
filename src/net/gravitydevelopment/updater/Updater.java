 ...
 * Updater for Bukkit.
 *
 * This class provides the means to safely and easily update a plugin, or check to see if it is
 * updated using dev.bukkit.org
 */
...
public class Updater {
  ...
  private static final String delimiter = "^v|[\\s_-]v"; // Used for locating version numbers in
                                                         // file names
  private static final String[] NO_UPDATE_TAG = {"-DEV", "-PRE", "-SNAPSHOT", "-TEKKIT"}; // If the
                                                                                          // version
                                                                                          // number
                                                                                          // contains
                                                                                          // one of
  ...
  public Updater(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
    ...
    try {
      ...
      plugin.getLogger().log(Level.SEVERE,
          "The project ID provided for updating, " + id + " is invalid.", e);
          ...
          if (this.pluginFile(dFile.getName())) {
            ...
            for (final File cFile : dFile.listFiles()) // Loop through all the files in the new dir
            {
              ...
              if (!found) {
                ...
                cFile.renameTo(
                    new File(oFile.getCanonicalFile() + File.separator + cFile.getName()));
              } else {
  ...
  private boolean versionCheck(String title) {
    if (this.type != UpdateType.NO_VERSION_CHECK) {
      ...
      if (title.split(delimiter).length == 2) {
        final String remoteVersion = title.split(delimiter)[1].split(" ")[0]; // Get the newest
                                                                              // file's version
                                                                              // number
