...
public class SignLoader {
  ...
  public synchronized void loadFile() {
    ...
    if (new File(plugin.getDataFolder(), "signs.dat").exists()) {
      ...
      new File(plugin.getDataFolder(), "signs.dat")
          .renameTo(new File(plugin.getDataFolder(), "signs.bak"));
    }

    FileConfiguration config =
        YamlConfiguration.loadConfiguration(new File(plugin.getDataFolder(), "signs.yml"));
    ...
    for (String key : data.getKeys(false)) {
      try {
        ...
        plugin.getLogger()
            .warning("Unable to load sign " + attempts + " in signs.yml. " + ex.getMessage());
        ex.printStackTrace();
      }
    }
    plugin.getLogger().info("Successfully loaded " + plugin.activeSigns.size() + " signs");
  }
      ...
      if (file.exists()) {
        ...
        while (scanner.hasNextLine()) {
          ...
          try {
            ...
            plugin.getLogger().warning("Unable to load sign in signs.dat line " + lineNumber);
          }
        }
        scanner.close();
        inStream.close();
        plugin.getLogger().info("Imported " + plugin.activeSigns.size() + " old signs");
      }
    } catch (Exception ex) {
      ex.printStackTrace();
    }
  ...
  public synchronized void saveFile() {
    ...
    for (Map.Entry<Location, SignText> sign : plugin.activeSigns.entrySet()) {
      ...
      cst.trim();
      String key = loc.getWorld().getName() + "," + loc.getBlockX() + "," + loc.getBlockY() + ","
          + loc.getBlockZ();
    ...
    try {
      ...
      for (Map.Entry<Location, SignText> entry : plugin.activeSigns.entrySet()) {
        try {
          ...
          if (csl != null)
            plugin.getLogger()
                .warning("Unable to save sign #" + signNumber + " at " + csl.toString());
          else
            plugin.getLogger().warning("Unable to save sign #" + signNumber);
        }
      }
      writer.close();
    } catch (Exception ex) {
