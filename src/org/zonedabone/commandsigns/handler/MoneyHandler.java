...
public class MoneyHandler extends Handler {
  ...
  @Override
  public void handle(SignExecutor e, String command, boolean silent, boolean negate) {
    plugin = e.getPlugin();
    if (e.getPlayer() != null && CommandSigns.economy != null && CommandSigns.economy.isEnabled()
        && command.startsWith("$")) {
      double amount = 0;
      try {
      ...
      if (CommandSigns.economy.withdrawPlayer(e.getPlayer(), amount).transactionSuccess()
          ^ negate) {
        ...
        if (!silent)
          plugin.messenger.sendMessage(e.getPlayer(), "restriction.not_enough_money",
              new String[] {"MONEY"}, new String[] {"" + CommandSigns.economy.format(amount)});
      }
    }
  }
